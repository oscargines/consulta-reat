...
│  HistorialScreen · AcercaDeScreen · AppNavigation      │
└──────────────▲──────────────────────▲──────────────────┘
...
- **`AppNavigation.kt`** – `NavHost` con las rutas `inicio`, `consultas`, `historial` y `acercade`. La barra inferior de navegación (Inicio, Consultas, `+`, Historial, Acerca de) solo se muestra fuera de la pantalla principal. El botón `+` abre una nueva consulta.
